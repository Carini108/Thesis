...
def main():
    ...
    E_thres = 1000.0 # energy threshold in GeV
    # read event file
    ...
    with alive_bar(n_events, force_tty=True) as bar: # to show the overall progress (must need n_events in advance)
        for iev, event in enumerate(F):
            ...
            if bool_bottom==False or bool_antilep==False or bool_neutr==False or bool_antibottom==False or bool_lep==False or bool_antineutr==False :
            ...
            # reconstruct the top and the antitop from their children
            top_momenta = bottom_momenta + antilep_momenta + neutr_momenta
            antitop_momenta = antibottom_momenta + lep_momenta + antineutr_momenta
